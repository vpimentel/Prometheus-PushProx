...
var (
	...
	baseContextPath      = kingpin.Flag("web.context-path", "Base context path address").Default("/").String()
)
...
func newHTTPHandler(logger log.Logger, coordinator *Coordinator, mux *http.ServeMux) *httpHandler {
	...
	pushPath := join(*baseContextPath, "push")
	pollPath := join(*baseContextPath, "poll")
	clientsPath := join(*baseContextPath, "clients")
	metricsPath := join(*baseContextPath, "metrics")

	// api handlers
	handlers := map[string]http.HandlerFunc{
		pushPath:    h.handlePush,
		pollPath:    h.handlePoll,
		clientsPath: h.handleListClients,
		metricsPath: promhttp.Handler().ServeHTTP,
	}
